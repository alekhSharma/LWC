...
export default class GoogleTypeAhead extends LightningElement {

    @api label="Search";
    @api selection = [];
    @api placeholder = "Search Salesforce";
...
// EXPOSED FUNCTIONS
    ...
    setSearchResults(results) {
        console.log('inside the setsearchResult');
                ...
                if (this.cleanSearchTerm.length >= MINIMAL_SEARCH_TERM_LENGTH) {
                    const searchEvent = new CustomEvent('search', {
                        detail: {
                            ...
                            selectedIds: this.selection.map(element => element.place_id)
                        }
                    });
...
// EVENT HANDLING
    ...
    handleResultClick(event) {
        ...
        // Save selection
        let selectedItem = this.searchResults.filter(result => result.place_id === recordId);
    ...
    handleRemoveSelectedItem(event) {
        ...
        this.selection = this.selection.filter(item => item.place_id !== recordId);
        // Notify parent components that selection has changed
        const selectionchange = new CustomEvent('selectionchange', {
            detail: ''
        });
...
// STYLE EXPRESSIONS
    ...
    get getComboboxClass() {
        ...
        if (this.isMultiEntry) {
           ...
           css += (this.hasSelection() ? 'slds-input-has-icon_right' : 'slds-input-has-icon_right');
        }
        return css;
    }
    ...
    get getInputValue() {
        ...
        return this.hasSelection() ? this.selection[0].description : this.searchTerm;
    }
